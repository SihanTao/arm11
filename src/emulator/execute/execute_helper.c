...
void execute_MUL(mul_t instruction, ArmState arm_state)
{
	...
	if (instruction.Rd == PC || instruction.Rm == PC || instruction.Rn == PC || instruction.Rs == PC)
	{
		perror("PC should not used as operand or destination register");
		exit(EXIT_FAILURE);
	}

	if (instruction.Rd == instruction.Rm)
	{
		perror("Rd will not be the same as Rm");
		exit(EXIT_FAILURE);
	}
