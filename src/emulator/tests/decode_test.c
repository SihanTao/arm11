...
int main(void)
{
  add_test("Test decode:");
  {
    ...
    test_int_v(instruction1.word.proc.cond, AL, "mov r1, #1 cond == always");
    test_int_v(instruction1.word.proc.operand2.rot_imm.imm, 1,
               "mov r1, #1 oprand2.imm = 1");
    test_int_v(instruction1.word.proc.operand2.rot_imm.amount, 0, "rotate amount = 0");
    test_int_v(instruction1.word.proc.Rd, 1, "mov r1, #1, Rn == 1");


    instruction1 = decode(fetched[1]);
    test_int_v(instruction1.tag, BRANCH, "b foo == branch");
    test_int_v(instruction1.word.branch.cond, AL, "b foo cond == always");
    test_int_v(instruction1.word.branch.offset, 0, "b foo cond == always");

    instruction1 = decode(fetched[2]);
    test_int_v(instruction1.tag, DATA_PROCESS, "mov r2, #2 == data processing");
    test_int_v(instruction1.word.proc.cond, AL, "mov r2, #2 cond == always");
    ...
    test_int_v(instruction1.tag, DATA_PROCESS, "foo: mov r3, #3 == data processing");
    test_int_v(instruction1.word.proc.cond, AL, "mov r3, #3 cond == always");
    ...
    test_int_v(instruction2.word.mul.cond, AL, "mul r2,r1,r0 cond == always");
    test_int_v(instruction2.word.mul.acc, 0, "mul r2,r1,r0 acc == 0");
    test_int_v(instruction2.word.mul.set_cond, 0, "mul r2,r1,r0 s == 0");
    ...
    test_int_v(instruction.word.trans.cond, AL, "ldr r2,[r0] cond == AL");
    ...
    test_int_v(instruction.word.trans.offset.rot_imm.imm, 0, "ldr r2,[r0] imm == 0");
    test_int_v(instruction.word.trans.offset.rot_imm.amount, 0, "ldr r2,[r0] amount == 0");
  }
